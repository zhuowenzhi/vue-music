<template>
  <div id="slider">
    <div class="window" @mouseover="stop" @mouseleave="play">
      ...
      <ul class="direction">
        <li class="left" @click="move(982, 1, speed)">
        ...
        <li class="right" @click="move(982, -1, speed)">
    ...
    animate (des, direc, speed) {
      ...
      this.temp = window.setInterval(() => {
        if ((direc === -1 && des < this.distance) || (direc === 1 && des > this.distance)) {
          ...
          window.clearInterval(this.temp)
          this.distance = des
          if (des < -3000) this.distance = -982
          if (des > -982) this.distance = -3000
        }
      }, 20)
    },
    jump (index) {
      const direction = index - this.currentIndex >= 0 ? -1 : 1
      const offset = Math.abs(index - this.currentIndex) * 982
